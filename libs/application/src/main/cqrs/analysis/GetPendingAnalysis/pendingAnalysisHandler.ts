...
import { PendingAnalysisQuery } from './pendingAnalysisquery';
...
@QueryHandler(PendingAnalysisQuery)
export class PendingAnalysisHandler implements IQueryHandler<PendingAnalysisQuery, Response<AnalysisModel>>{
    constructor(
        @Inject('IAnalysisDomain')
        private readonly analysis: IAnalysisDomain
    ) { }
    async execute(query: PendingAnalysisQuery): Promise<Response<AnalysisModel>> {
        const processAnalysisRequest = query.request;
        const analysisMode: AnalysisModel = {
            ...
            P_IDENTIFICADOR: processAnalysisRequest.Idetificacion,
        ...
        if (query.request.AwaitResponse) {
            return await this.analysis.ProcessAnalysisWaitResponseAsync(analysisMode)
        } else {
            return await this.analysis.GetPendingAnalisysListAsync(analysisMode)
        }
    }
}