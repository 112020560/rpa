...
import { MasiveAnalysisCommand } from './processmasive.analysis.command';
...
@CommandHandler(MasiveAnalysisCommand)
export class MasiveAnalysisHandler implements ICommandHandler<MasiveAnalysisCommand, Response<AnalysisModel>>{
    constructor(
        @Inject('IAnalysisDomain')
        private readonly analysis: IAnalysisDomain
    ) { }
    async execute(command: MasiveAnalysisCommand): Promise<Response<AnalysisModel>> {
        //console.log(command.request.ProcessAnalysisRequestsList);
        const analysisModels: AnalysisModel[] = []
        command.request.ProcessAnalysisRequestsList.forEach(processAnalysisRequest => {
            analysisModels.push({
        ...
        return await this.analysis.ProcessMasiveAnalysisAsync(analysisModels);
    }
}